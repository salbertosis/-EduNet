...
impl GeneradorResumenFinal {
    ...
    fn generar_html_resumen_final(&self, estudiantes: &[EstudianteResumenFinal], datos_institucion: &DatosInstitucion, tipo_evaluacion: &str, ano_escolar: &str, asignaturas: &[Asignatura], id_grado: i32) -> Result<String, String> {
        ...
        self.reemplazar_placeholders_asignaturas(&mut html_content, asignaturas, tipo_evaluacion, id_grado, datos_institucion, estudiantes.len(), ESTUDIANTES_POR_PAGINA);
    ...
    fn reemplazar_placeholders_asignaturas(&self, html_content: &mut String, asignaturas: &[Asignatura], tipo_evaluacion: &str, id_grado: i32, datos_institucion: &DatosInstitucion, total_estudiantes: usize, estudiantes_por_pagina: usize) {
        ...
        self.generar_tercera_tabla_dinamica(html_content, asignaturas, id_grado, total_estudiantes, estudiantes_por_pagina);
        
        // Reemplazar placeholder del plan de estudio
        *html_content = html_content.replace("{{PLAN_ESTUDIO}}", "");
...
fn generar_tercera_tabla_dinamica(&self, html_content: &mut String, asignaturas: &[Asignatura], id_grado: i32, total_estudiantes: usize, estudiantes_por_pagina: usize) {
    let mut filas_tercera_tabla = String::new();
    
    // Generar exactamente 12 filas según la imagen
    for i in 0..12 {
        let numero = format!("{:02}", i + 1);
        
        // Determinar el contenido específico para cada fila según la imagen
        let (codigo_fila, nombre_fila) = match i {
            0 => ("CA", "CASTELLANO"),
            1 => ("ILE", "INGLÉS Y OTRAS LENGUAS EXTRANJERAS"),
            2 => ("MA", "MATEMÁTICAS"),
            3 => ("EF", "EDUCACIÓN FÍSICA"),
            4 => ("FI", "FÍSICA"),
            5 => ("QU", "QUÍMICA"),
            6 => ("BI", "BIOLOGÍA"),
            7 => ("GHC", "GEOGRAFÍA, HISTORIA Y CIUDADANÍA"),
            8 => ("FN", "FORMACIÓN PARA LA SOBERANÍA NACIONAL"),
            9 => ("OC", "ORIENTACIÓN Y CONVIVENCIA"),
            10 => ("PGCRP", "PARTICIPACIÓN EN GRUPOS DE CREAC., RECREAC. Y"),
            11 => ("*", "*"),
            _ => ("*", "*"),
        };
        
        // Preparar los valores para las columnas de estudiantes
        ...
        // Determinar si esta fila debe tener rowspan para las columnas de estudiantes
        ...
        // Determinar si esta fila debe tener colspan para combinar columnas 7 y 8
        ...
        // Para la fila 7 (índice 6), crear un bloque combinado con rowspan y colspan
        ...
        <td style="border: 1px solid #000; font-size: 8px; text-align: center; padding: 2px; font-weight: bold;"{}>{}</td>
                ...
                numero, codigo_fila, nombre_fila, colspan_7_8, ""
            )
        } else if i == 6 {
        ...
        <td style="border: 1px solid #000; font-size: 8px; text-align: center; padding: 2px; font-weight: bold;"{}>{}</td>
                ...
                numero, codigo_fila, nombre_fila, rowspan_colspan_7_8, "PLAN DE ESTUDIO"
            )
        } else {
